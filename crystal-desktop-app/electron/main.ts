...
const PROTOCOL_NAME = 'crystalapp';

/**
 * Handles authentication callback from deep link URL.
 * Parses the crystalapp:// URL and extracts the sign-in ticket,
 * then sends it to the renderer process via IPC.
 * 
 * @param url - The deep link URL containing the auth callback
 */
function handleAuthCallback(url: string) {
  ...
  try {
    ...
    if (ticket && win) {
      console.log('[Auth] Ticket received, sending to renderer');
...
// Ensure single instance and handle deep links on Windows/Linux
...
if (!gotTheLock) {
  ...
  app.on('second-instance', (_event, argv) => {
    // Windows/Linux: deep link URL is passed as command line argument
...
ipcMain.on("open-external", (_event, url: string) => {
  console.log('[App] Opening external URL:', url);
...
app.whenReady().then(() => {
  try {
    ...
    console.log('[App] Registered protocol:', PROTOCOL_NAME);
