import require$$1$4, { app, ipcMain, desktopCapturer, BrowserWindow, dialog, screen } from "electron";
...
function createWindow() {
  try {
    ...
    win.webContents.on("will-navigate", (event, url) => {
      ...
      const isLocalAppUrl = url.includes("localhost:5173") || url.includes("127.0.0.1:5173");
      if (isLocalAppUrl) {
        return;
      }
    ...
    console.error("[CreateWindow] Error:", error2);
    dialog.showErrorBox("Crystal Error", `Failed to create window: ${error2}`);
  }
}
...
app.whenReady().then(() => {
  try {
    ...
    console.error("[App] Startup error:", error2);
    dialog.showErrorBox("Crystal Error", `Failed to start: ${error2}`);
  }
});
...
process.on("uncaughtException", (error2) => {
  console.error("[Process] Uncaught exception:", error2);
  dialog.showErrorBox("Crystal Error", `Unexpected error: ${error2.message}`);
});
