 ...
 * 
 * Listens for auth-callback IPC messages from the main process and exchanges
 * the sign-in ticket for a Clerk session. This completes the browser-based
    ...
    const handleAuthCallback = async (_event: unknown, payload: { ticket: string }) => {
      ...
      try {
        ...
        if (result.status === "complete" && result.createdSessionId) {
          ...
          console.log("[Auth] Session activated successfully");
        }
      } catch (error) {
        console.error("[Auth] Failed to exchange ticket:", error);
      }
    };
